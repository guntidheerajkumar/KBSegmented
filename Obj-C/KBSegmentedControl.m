...
@property (nonatomic, strong) KBScrollView *scrollView;
...
- (id)initWithSectionTitles:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.type = KBSegmentedControlTypeText;
    }
...
- (id)initWithSectionImages:(NSArray*)sectionImages sectionSelectedImages:(NSArray*)sectionSelectedImages {
    ...
    if (self) {
        ...
        self.type = KBSegmentedControlTypeImages;
    }
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.sectionSelectedImages = sectionSelectedImages;
        self.sectionTitles = sectiontitles;
        self.type = KBSegmentedControlTypeTextImages;
    }
...
- (void)commonInit {
    self.scrollView = [[KBScrollView alloc] init];
    ...
    self.opaque = NO;
    _selectionIndicatorColor = self.selectionIndicatorColor;
    ...
    self.selectionStyle = KBSegmentedControlSelectionStyleTextWidthStripe;
    self.selectionIndicatorLocation = KBSegmentedControlSelectionIndicatorLocationDown;
    self.segmentWidthStyle = KBSegmentedControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(KBSegmentedControlSelectionIndicatorLocation)selectionIndicatorLocation {
    _selectionIndicatorLocation = selectionIndicatorLocation;
    
    if (selectionIndicatorLocation == KBSegmentedControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(KBSegmentedControlSegmentWidthStyle)segmentWidthStyle {
    // Force KBSegmentedControlSegmentWidthStyleFixed when type is KBSegmentedControlTypeImages.
    if (self.type == KBSegmentedControlTypeImages) {
        _segmentWidthStyle = KBSegmentedControlSegmentWidthStyleFixed;
    } else {
        _segmentWidthStyle = segmentWidthStyle;
    }
}

- (void)setBorderType:(KBSegmentedControlBorderType)borderType {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == KBSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            BOOL locationUp = (self.selectionIndicatorLocation == KBSegmentedControlSelectionIndicatorLocationUp);
            BOOL selectionStyleNotBox = (self.selectionStyle != KBSegmentedControlSelectionStyleBox);
            ...
            CGRect rect;
            if (self.segmentWidthStyle == KBSegmentedControlSegmentWidthStyleFixed) {
            ...
            } else if (self.segmentWidthStyle == KBSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == KBSegmentedControlTypeImages) {
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == KBSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
    ...
    } else if (self.type == KBSegmentedControlTypeTextImages){
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
    ...
    if (self.selectedSegmentIndex != KBSegmentedControlNoSegment) {
        if (self.selectionStyle == KBSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                ...
                if (self.selectionStyle == KBSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    // Border layer
    if (self.borderType & KBSegmentedControlBorderTypeTop) {
...
- (void)setArrowFrame {
    ...
    if (self.selectionIndicatorLocation == KBSegmentedControlSelectionIndicatorLocationDown) {
    ...
    if (self.selectionIndicatorLocation == KBSegmentedControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == KBSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == KBSegmentedControlSelectionStyleTextWidthStripe &&
            sectionWidth <= self.segmentWidth &&
            self.segmentWidthStyle != KBSegmentedControlSegmentWidthStyleDynamic) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
            ...
            if (self.segmentWidthStyle == KBSegmentedControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
    ...
    if (self.type == KBSegmentedControlTypeText && self.segmentWidthStyle == KBSegmentedControlSegmentWidthStyleFixed) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == KBSegmentedControlTypeText && self.segmentWidthStyle == KBSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == KBSegmentedControlTypeTextImages && self.segmentWidthStyle == KBSegmentedControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == KBSegmentedControlTypeTextImages && self.segmentWidthStyle == KBSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        } else if (self.type == KBSegmentedControlTypeTextImages || self.type == KBSegmentedControlTypeText) {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if (index == KBSegmentedControlNoSegment) {
        ...
        if (animated) {
            ...
            if(self.selectionStyle == KBSegmentedControlSelectionStyleArrow) {
                ...
                if ([self.selectionIndicatorStripLayer superlayer] == nil) {
                    ...
                    if (self.selectionStyle == KBSegmentedControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
                        [self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
